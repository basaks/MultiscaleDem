...
export WHITEBOX_TOOLS=/home/ubuntu/whitebox-tools/target/release/whitebox_tools
echo ${WHITEBOX_TOOLS}
...
maxeledev(){
    ...
    ${WHITEBOX_TOOLS} -r=MinSignedElevationDeviation -v --dem=$PWD/${flt}.flt \
        -out_mag=$PWD/${flt}_min_mag${scale}.flt \
        --out_scale=$PWD/${flt}_min_scale${scale}.flt \
        --min_scale=${min_scale} --max_scale=${max_scale} --step=${step}

    # remove the unused _scale file
    rm $PWD/${flt}_min_scale${scale}.*
}
